...
void build_submatrices(Mat mats[],equil_fields eq,geom_shape* geom,matrix_coeffs* coeffs,gq_mod* gq_mod,int main_mod,int sec_mod,int row_offset,int col_offset)
{
  ...
  if( geom->Hall_on == 1 ){
	...
	dw_dh[0]=false; dw_dh[1]=true;
	fill_submatrix(mats[1],geom,gq_mod,eq.rad_interp,coeffs->f_pdp,geom->ind_perp_main[i],geom->ind_perp_sec[j],dim_sub,row_offset,col_offset,dw_dh,ADD_VALUES);
	...
	dw_dh[0]=true; dw_dh[1]=true; 
	fill_submatrix(mats[1],geom,gq_mod,eq.rad_interp,coeffs->f_dwdp,geom->ind_wedge_main[i],geom->ind_perp_sec[j],dim_sub,row_offset,col_offset,dw_dh,ADD_VALUES);

	dw_dh[0]=true; dw_dh[1]=false; 
	fill_submatrix(mats[1],geom,gq_mod,eq.rad_interp,coeffs->f_dwp,geom->ind_wedge_main[i],geom->ind_perp_sec[j],dim_sub,row_offset,col_offset,dw_dh,ADD_VALUES);
	
	dw_dh[0]=false; dw_dh[1]=true; 
	fill_submatrix(mats[1],geom,gq_mod,eq.rad_interp,coeffs->f_wdp,geom->ind_wedge_main[i],geom->ind_perp_sec[j],dim_sub,row_offset,col_offset,dw_dh,ADD_VALUES);
	...
	dw_dh[0]=true; dw_dh[1]=false; 
	fill_submatrix(mats[1],geom,gq_mod,eq.rad_interp,coeffs->f_dww,geom->ind_wedge_main[i],geom->ind_wedge_sec[j],dim_sub,row_offset,col_offset,dw_dh,ADD_VALUES);
